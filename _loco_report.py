...
loco_res_trips = loco_res_start[['loco', 'st_from_name', 'st_to_name', 'time_start', 'time_start_norm', 'train']].                set_index(['loco', 'train']).join(loco_res_end[['loco', 'st_to_name', 'time_end', 'time_end_norm', 'train']].                                                  set_index(['loco', 'train']), rsuffix='_end').reset_index()[cols]
...
loco_res_trips['back_to_reg'] = loco_res_trips.apply(lambda row:                                                     not any([int(r) in [int(x) for x in row.loco_reg] for r in row.reg]),
                                                     axis=1)
