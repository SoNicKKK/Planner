...
# 
# 1. Сборка джарника делается батником `build.bat`. Он очищает папку `C:\work\UniversalPlanner`, клонирует проект из гита в нее же, собирает его maven'ом и копирует готовый джарник в `C:\work`. В процессе работы потребуется ввести пароль для гита для пользователя `jenkins`: пароль - `jenkins123`.
