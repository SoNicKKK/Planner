...
# ## Баги
# 
# 1. Когда вручную создается древняя явка бригады, то в time_start проставляется id станции. Пример: бригада 9200022 на модельном примере. Должно подставляться время из `fact_team_ready`.
# 2. Неправильно считается рабочее время для таких бригад (у которых была древняя явка). Пример: та же бригада, которая хреначит без отдыха.
# 3. Много бригад (в основном, в state = 3) вообще никак не участвуют в планировании, их нет в System.out.log, но их нет и в логе фильтрации. Примеры: 9244013...9244046.
